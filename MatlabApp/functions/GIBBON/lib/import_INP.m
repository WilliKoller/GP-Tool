...
targets={'*NODE','*ELEMENT', '*NSET, NSET=GP_', '*NSET, NSET=GP_Start'}; %Text form targets
    ...
    if gcontains(l,targets,'IgnoreCase',true)
        ...
        switch targetIndex
            ...
            case 3 %Nodeset               
                siz=[numel(T),6];
                D=readLines(T,lineCount+1,siz);
                logicKeep=~any(isnan(D(:, 1)),2);
                NodeSetTemp=D(logicKeep,:);
                NodeSetTemp = sort(reshape(NodeSetTemp,1,[]))';
                NodeSet = NodeSetTemp(~isnan(NodeSetTemp));
            case 4 %starting Node               
                siz=[numel(T),1];
                D=readLines(T,lineCount+1,siz);
                logicKeep=~any(isnan(D),2);
                startingNode=D(logicKeep,:);
        end        
    end    
...
nodeStruct.N=N;
nodeStruct.N_ind=nodeIndex(:);
if exist('NodeSet', 'var') 
    nodeStruct.Nodeset=NodeSet;
end
if exist('startingNode', 'var') 
    nodeStruct.startingNode=startingNode;
end
...
for q=lineCount:numel(T)
    try          
        ...
        temp = sscanf(T{q},formatSpec);
        if size(temp, 1) > 0
            D(q,1:size(temp, 1)) = temp;
        end
        break
    end
end
